...
export class SecurityScanner {
   ...
   */
  async scanContract(contractPath: string): Promise<SecurityReport> {
   ...
   */
  private async checkReentrancy(): Promise<void> {
   ...
   */
  private async checkUncheckedExternalCalls(): Promise<void> {
   ...
   */
  private async checkIntegerOverflow(): Promise<void> {
    ...
    const solidityVersion = hasSolidityVersion?.[1] || '0.8.0';
   ...
   */
  private async checkUnprotectedSelfdestruct(): Promise<void> {
   ...
   */
  private async checkDelegatecallToUntrusted(): Promise<void> {
   ...
   */
  private async checkTimestampDependence(): Promise<void> {
   ...
   */
  private async checkRandomnessIssues(): Promise<void> {
   ...
   */
  private async checkGasLimitIssues(): Promise<void> {
   ...
   */
  private async checkUninitializedStorage(): Promise<void> {
   ...
   * @param lineNumber - Line number to get context for
   * @returns Function context
   */
  private getFunctionContext(lineNumber: number): string {
   ...
   * @param lineNumber - Line number to get function body for
   * @returns Function body
   */
  private getFunctionBody(lineNumber: number): string {
    const lines = this.sourceCode.split('\n');
    let braceCount = 0;
   ...
   */
  checkSeverity(issue: SecurityIssue): number {
    const severityMap = {
      'critical': 4,
      'high': 3,
      'medium': 2,
  ...
  displayReport(report: SecurityReport): void {
    ...
    if (report.issues.length > 0) {
      ...
      report.issues.forEach((issue, index) => {
        ...
        const severityColor = severityColorMap[issue.severity] || chalk.white;
    ...
    if (report.recommendations.length > 0) {
      ...
      report.recommendations.forEach((rec, index) => {
        ...
        const severityColor = severityColorMap[rec.severity] || chalk.white;
...
export async function scanContract(contractPath: string): Promise<SecurityReport> {
